...
export const insertUserContactMessageData = async (data) => {
  ...
  const result = await pool.query(
    `
      INSERT INTO my_portfolio.users_contact_messages
      (
        id,
        name,
        email,
  ...
  Promise.allSettled([
    ...
    sendMailToUserForSuccesSentContact({
      name,
      email,
      mobile,
      message
...
export const userContactMessageData = async (search) => {
  ...
  let query = `
    SELECT *
    FROM my_portfolio.users_contact_messages
    WHERE 1 = 1
  `;
...
export const markUserContactMessageAsRead = async (id) => {

  const result = await pool.query(
    `
      UPDATE my_portfolio.users_contact_messages
